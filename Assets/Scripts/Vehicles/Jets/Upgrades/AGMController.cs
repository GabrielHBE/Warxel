...
public class AGMController : MonoBehaviour, JetUpgradeController
{
    ...
    private List<Agm> agms = new List<Agm>();
    private int current_rocket_index = 0;

    private Agm current_agm;
    ...
    void Update()
    {
        ...
        if (agms.Count < spawnPoints.Count)
        {
        ...
        {
            if (hit.collider.gameObject.tag == "GroundVehicle")
            {
    ...
    void InitializeMissiles()
    {
        foreach (Transform spawnPoint in spawnPoints)
        {
            ...
            Agm rocket = currentItem.GetComponent<Agm>();
    ...
    {
        if (current_rocket_index < agms.Count - 1)
        {
            current_rocket_index++;
            current_agm = agms[current_rocket_index];
        }
        else
        {
            agms.Clear();
